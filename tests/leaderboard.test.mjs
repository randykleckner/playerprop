...
test('API retrieves exact historical week, validates selectors and shows pending',async()=>{const {env}=await ready();for(const [query,status,week] of [['season=2026&week=1','ready',1],['season=2026&week=2','ready',2],['season=2025&week=1','pending',1]]){const r=await leaderboardRoute(new Request(`https://local/api/leaderboard?${query}`),env);const body=await r.json();assert.equal(body.status,status);assert.equal(body.week,week);assert.equal(body.available.length,2);}assert.equal((await leaderboardRoute(new Request('https://local/api/leaderboard?season=wat&week=1'),env)).status,400);});
...
test('database actuals render without a finalized week or snapshot, and choose latest stored week',async()=>{
 const {env,sql}=fixture();const body=await(await leaderboardRoute(new Request('https://local/api/leaderboard'),env)).json();
 assert.equal(body.status,'ready');assert.equal(body.week,2);assert.equal(body.dataSource,'database');assert.equal(body.scoringComplete,false);assert.equal(body.leaders.length,4);assert.deepEqual(body.missingPositions,['DST']);assert.equal(body.hero.fantasyPoints,28.36);
 assert.equal(sql.prepare('SELECT COUNT(*) n FROM weekly_leaderboards').get().n,0);
});
test('the original database schema works without any leaderboard migration',async()=>{
 const {env,sql}=fixture();sql.exec('DROP TABLE weekly_leaderboards;DROP TABLE leaderboard_week_readiness;DROP TABLE leaderboard_hero_media;DROP TABLE team_game_fantasy_stats;ALTER TABLE player_game_stats DROP COLUMN passing_interceptions;ALTER TABLE player_game_stats DROP COLUMN fumbles_lost;');
 const response=await leaderboardRoute(new Request('https://local/api/leaderboard?season=2026&week=1'),env);assert.equal(response.status,200);const body=await response.json();assert.equal(body.status,'ready');assert.equal(body.leaders.length,4);assert.equal(body.scoringComplete,false);
});
test('partial team defense data never ranks a fabricated DST leader',async()=>{
 const {env,sql}=fixture();sql.exec("INSERT INTO team_game_fantasy_stats VALUES ('game-1','CHI',2,1,0,0,14)");const body=await(await leaderboardRoute(new Request('https://local/api/leaderboard?season=2026&week=1'),env)).json();assert.deepEqual(body.missingPositions,['DST']);
});
