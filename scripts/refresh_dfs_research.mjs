...
export function refreshResearch(salary,projection,schedule,{root='public',now=new Date().toISOString(),markets=[],marketFetchedAt=null,identityEvidence=null}={}){
 ...
 const sourceKey=sha({salary,projection,schedule,markets,marketFetchedAt,identityEvidence});
 ...
 for(const row of identities){const evidence=identityEvidence?.rows.find(r=>r.draftable_id===row.draftable_id);if(evidence){row.review_evidence=evidence;row.priority=evidence.priority;}}
 ...
 const readiness=[
  ...
  {label:'Player mapping',detail:`Eligible offense: ${verified} verified, ${provisional} provisional; ${pool.filter(p=>p.position==='DST').length} team defenses. ${input.players.filter(p=>p.identity_review?.status==='review_required').length} eligible identities have context discrepancies. Full offensive pool: ${counts.unresolved} unresolved, ${counts.ambiguous} ambiguous`},
 ...
 const files={'simulation.json':input,'lineups.json':daily,'identities.json':{slate_id:input.slate_id,generated_at:now,counts,eligible:{verified,provisional,dst:pool.filter(p=>p.position==='DST').length},exclusions:rejected,evidence_sources:identityEvidence?.sources||null,rows:identities}};
