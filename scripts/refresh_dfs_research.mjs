...
import {confidenceFor,tierCounts} from './dfs-js/identity-confidence.mjs';
...
export function refreshResearch(salary,projection,schedule,{root='public',now=new Date().toISOString(),markets=[],marketFetchedAt=null,identityEvidence=null}={}){
 ...
 const sourceKey=sha({salary,projection,schedule,markets,marketFetchedAt,identityEvidence,identityPolicy:'nflverse-v2-context-v1'});
 ...
 if(identityEvidence){
  if(identityEvidence.version!==1||identityEvidence.verified_by_this_review!==0)throw Error('Invalid non-verifying identity evidence report');
  for(const p of input.players){const evidence=identityEvidence.rows.find(r=>r.draftable_id===p.draftable_id&&r.candidate_player_id===p.player_id);if(!evidence)continue;p.identity_review=evidence.audit;p.quality_flags=p.quality_flags.filter(f=>f!=='identity_evidence_review_required');if(evidence.audit?.status==='review_required')p.quality_flags.push('identity_evidence_review_required');}
  input.sources.identity_evidence=identityEvidence.sources;
 }
 const projections=new Map(projection.records.map(p=>[p.player_id,p]));
 for(const p of pool){const e=identityEvidence?.rows.find(r=>r.draftable_id===p.draftable_id);const result=confidenceFor(p,e,projections.get(p.id),schedule,identityEvidence?.manual_overrides||[],now);p.identity_confidence=result.tier;p.identity_evidence=result;
  const record=input.players.find(x=>x.player_id===p.id);record.identity_confidence=result.tier;record.identity_evidence=result;
  if(result.tier==='strongly_corroborated'){p.provisional=false;record.identity_status='strongly_corroborated';if(record.identity_review)record.identity_review={...record.identity_review,status:'strongly_corroborated',override_id:result.override_id};for(const row of [p,record])row.quality_flags=row.quality_flags.filter(f=>!['mapping_review_required','unresolved_player_mapping','identity_evidence_review_required'].includes(f));}
 }
 const tiers=tierCounts(input.players);input.coverage.identity_tiers=tiers;input.coverage.provisional=tiers.provisional;
 input.assumptions=input.assumptions.filter(x=>!x.startsWith('Provisional identity matches'));
 input.assumptions.push(`Identity tiers: ${tiers.verified} verified, ${tiers.strongly_corroborated} strongly corroborated, ${tiers.provisional} provisional, ${tiers.unresolved} unresolved offensive entries. Strong corroboration is contextual evidence, not a provider-issued DK ID bridge.`);
 ...
 for(const row of identities){const member=pool.find(p=>p.draftable_id===row.draftable_id);if(member){row.confidence=member.identity_confidence;row.tier_evidence=member.identity_evidence;}const evidence=identityEvidence?.rows.find(r=>r.draftable_id===row.draftable_id);if(evidence){row.review_evidence=evidence;row.priority=evidence.priority;}}
 const verified=tiers.verified,provisional=tiers.provisional;
 const counts=Object.fromEntries(['verified','strongly_corroborated','provisional','unresolved'].map(k=>[k,identities.filter(r=>r.confidence===k).length]));
 const readiness=[
  ...
  {label:'Player mapping',detail:`Eligible offense: ${verified} verified, ${tiers.strongly_corroborated} strongly corroborated, ${provisional} provisional, ${tiers.unresolved} unresolved; ${pool.filter(p=>p.position==='DST').length} team defenses. ${input.players.filter(p=>p.identity_evidence?.override_id).length} documented overrides applied. Full offensive pool: ${counts.unresolved} unresolved`},
 ...
 const daily={version:1,model:'research-lineups-v1',generated_at:now,expires_at:input.expires_at,season:projection.season,week:projection.week,slate:salary.slate,source:'espn',scoring_basis:projection.scoring_basis,source_count:1,sources:{salary:{id:salary.snapshot_id,fetched_at:salary.fetched_at},projections:{fetched_at:projection.fetched_at},schedule:{source:'ESPN scoreboard',sha256:sha(schedule)}},coverage:{salary_players:salary.records.length,eligible_players:pool.length,provisional_players:provisional,identity_tiers:tiers,rejected},research_only:true,provisional_enabled:true,lineups};
 const bundle=sha({input,daily,identities}).slice(0,24),base=`/research/bundles/${bundle}`;
 const files={'simulation.json':input,'lineups.json':daily,'identities.json':{slate_id:input.slate_id,generated_at:now,counts,eligible:{...tiers,dst:pool.filter(p=>p.position==='DST').length},exclusions:rejected,evidence_sources:identityEvidence?.sources||null,rows:identities}};
