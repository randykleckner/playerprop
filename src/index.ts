...
async function upsertStats(db: D1Database, stats: PlayerStatInput[]) {
  ...
  const statements: D1PreparedStatement[] = [
    ...
    ...stats.map((stat) => db.prepare(`INSERT INTO player_game_stats (player_id, game_id, team_id, opponent_team_id, position, passing_yards, passing_touchdowns, rushing_yards, rushing_touchdowns, receptions, receiving_yards, receiving_touchdowns, targets, carries, tackles_solo, tackle_assists, tackles_for_loss, sacks, qb_hits, interceptions, passes_defended, passing_interceptions, fumbles_lost)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(player_id, game_id) DO UPDATE SET team_id = excluded.team_id, opponent_team_id = excluded.opponent_team_id, position = excluded.position, passing_yards = excluded.passing_yards, passing_touchdowns = excluded.passing_touchdowns, rushing_yards = excluded.rushing_yards, rushing_touchdowns = excluded.rushing_touchdowns, receptions = excluded.receptions, receiving_yards = excluded.receiving_yards, receiving_touchdowns = excluded.receiving_touchdowns, targets = excluded.targets, carries = excluded.carries, tackles_solo = excluded.tackles_solo, tackle_assists = excluded.tackle_assists, tackles_for_loss = excluded.tackles_for_loss, sacks = excluded.sacks, qb_hits = excluded.qb_hits, interceptions = excluded.interceptions, passes_defended = excluded.passes_defended, passing_interceptions = COALESCE(excluded.passing_interceptions, player_game_stats.passing_interceptions), fumbles_lost = COALESCE(excluded.fumbles_lost, player_game_stats.fumbles_lost), updated_at = CURRENT_TIMESTAMP`)
      .bind(stat.player_id, stat.game_id, stat.team, stat.opponent, stat.position, number(stat.passing_yards), number(stat.passing_touchdowns), number(stat.rushing_yards), number(stat.rushing_touchdowns), number(stat.receptions), number(stat.receiving_yards), number(stat.receiving_touchdowns), number(stat.targets), number(stat.carries), number(stat.tackles_solo), number(stat.tackle_assists), number(stat.tackles_for_loss), number(stat.sacks), number(stat.qb_hits), number(stat.interceptions), number(stat.passes_defended), nullableNumber(stat.passing_interceptions), nullableNumber(stat.fumbles_lost)))
  ];
