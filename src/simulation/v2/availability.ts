...
export interface State {retained?:boolean;state:Status;practice_status:string|null;game_status:string|null;source:string;updated_at:string;confidence:string;}
...
export function participation(p:Person,c:AvailabilityConfig,override?:Scenarios[string]):Participation{
 ...
 return {...p.availability,state,official_state:p.availability.state,active_probability:prob,workload_multiplier:work,participation:prob*work,scenario:!!override,basis:override?'User scenario; official evidence unchanged':p.availability.retained?'Last known injury report retained; current clearance unconfirmed':p.availability.game_status?'Official game designation; numeric participation probability is an unfitted scenario prior':p.availability.practice_status?'Official practice participation; workload reduction is a model assumption, not a confirmed gameday restriction':p.status==='ACT'?'Active roster only; no matched game designation. Baseline workload retained with low confidence':'Roster designation excludes expected gameday participation'};
}
