...
export function runFantasyBulk(game:GameInputV2,o:FantasyOptions,count:number,onProgress?:(done:number)=>void){
 ...
 const start=performance.now(),fields=Object.keys(emptyPlayerStats()),stats:Record<string,Record<string,Float64Array>>={},players:Record<string,PlayerInput>={};
...
export function compareFantasyRuns(base:FantasyRun,personnel:FantasyRun){
 if(base.metadata.engine!=='base'||personnel.metadata.engine!=='personnel'||base.seed!==personnel.seed||base.count!==personnel.count||fingerprint(base.game)!==fingerprint(personnel.game)||['player_snapshot','empirical_profile','madden_snapshot','roster_snapshot'].some(k=>base.metadata[k as keyof typeof base.metadata]!==personnel.metadata[k as keyof typeof personnel.metadata]))throw Error('Comparison requires identical matchup, count, seed and source snapshots');
