...
export function adjustments(offense:string,defense:string,team:PlayerTeam,personnel:PersonnelSnapshot,c:InfluenceConfig,level:number):Adjustments{
 validateInfluence(c,level);const flags:string[]=[];
 const difference=(a:string,b:string)=>{const x=personnel.units[offense]?.[a],y=personnel.units[defense]?.[b];if(!x?.complete||!y?.complete||x.rating===null||y.rating===null){flags.push(`${a}/${b}: incomplete evidence; adjustment disabled`);return null;}if(![x.rating,y.rating].every(n=>Number.isFinite(n)&&n>=0&&n<=99))throw Error('Invalid personnel unit');return x.rating-y.rating;};
 ...
 if(mass>=c.minimumReceivingCoverage&&secondary?.complete&&secondary.rating!==null)receivingDifference=(receiving/mass-secondary.rating)*mass;else flags.push('Receiving/secondary: incomplete evidence; adjustment disabled');
 if(missing>0)flags.push(`Receiving Madden coverage ${(100*mass).toFixed(1)}%; ${receivingDifference===null?'receiving modifier disabled':'modifier scaled to observed share'}`);
 return {protectionDifference,runDifference,receivingDifference,receivingCoverage:mass,pressureDelta:clamp(-(protectionDifference??0)*c.pressurePerRatingPoint*level,-c.maxPressureDelta,c.maxPressureDelta),completionDelta:clamp((receivingDifference??0)*c.completionPerRatingPoint*level,-c.maxCompletionDelta,c.maxCompletionDelta),runTilt:clamp((runDifference??0)*c.runShapePerRatingPoint*level,-c.maxShapeTilt,c.maxShapeTilt),completionTilt:clamp((receivingDifference??0)*c.completionShapePerRatingPoint*level,-c.maxShapeTilt,c.maxShapeTilt),flags};
}
