...
export function calibrationReport(cases,{trainingWindow,evaluationWindow}={}) {
  ...
  const seen=new Set();for(const r of cases){
    if(!r.player_id||!r.game_id||!['QB','RB','WR','TE','DST'].includes(r.position)||['mean','actual','p10','p90','stddev'].some(k=>!Number.isFinite(r[k]))||r.stddev<=0||r.p10>r.p90||(r.p75!=null&&(!Number.isFinite(r.p75)||r.p75<r.p10||r.p75>r.p90)))throw Error('Invalid held-out observation');
  ...
  const metrics=rows=>({count:rows.length,mean_error:rows.reduce((s,r)=>s+r.mean-r.actual,0)/rows.length,rmse:Math.sqrt(rows.reduce((s,r)=>s+(r.mean-r.actual)**2,0)/rows.length),p75_observations:rows.filter(r=>Number.isFinite(r.p75)).length,above_p75_pct:rows.some(r=>Number.isFinite(r.p75))?100*rows.filter(r=>Number.isFinite(r.p75)&&r.actual>r.p75).length/rows.filter(r=>Number.isFinite(r.p75)).length:null,variance_ratio:rows.reduce((s,r)=>s+(r.mean-r.actual)**2,0)/rows.reduce((s,r)=>s+r.stddev**2,0),mae:rows.reduce((s,r)=>s+Math.abs(r.mean-r.actual),0)/rows.length,above_p90_pct:100*rows.filter(r=>r.actual>r.p90).length/rows.length,below_p10_pct:100*rows.filter(r=>r.actual<r.p10).length/rows.length,mean_squared_standardized_error:rows.reduce((s,r)=>s+(r.actual-r.mean)**2/Math.max(r.stddev**2,1e-9),0)/rows.length});
