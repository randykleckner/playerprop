...
async function load() {
  ...
  try {
    ...
    $('lineup-status').innerHTML = manifest?readinessHtml(manifest):`${stale ? '<strong class="archived-label">STALE SNAPSHOT — refresh required.</strong> The salary or projection source has expired. ' : '<strong>Research preview.</strong> '}${data.coverage.eligible_players} of ${data.coverage.salary_players} salary entries passed the research-pool checks. ESPN is the only projection source so far. Player identity joins remain provisional; no top-10% or win probabilities are available. <a href="#methodology">View methodology</a>.`;
