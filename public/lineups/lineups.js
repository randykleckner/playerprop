import {avatar} from '../ui/components.js?v=ui-polish-2';
...
function card(lineup) {
  ...
  return `<article class="lineup-card ${featured ? 'featured' : ''}" aria-labelledby="title-${escape(lineup.id)}"><div class="lineup-card-head"><h3 id="title-${escape(lineup.id)}">${escape(lineup.title)}</h3><div class="lineup-stats"><div><strong class="points">${lineup.points.toFixed(2)}</strong><span>PROJECTED PTS</span></div><div><strong>${money(lineup.salary)}</strong><span>TOTAL SALARY</span></div></div>${featured ? explanation : ''}<div class="ui-lineup-portraits">${lineup.players.filter(p=>!lineup.required?.length||lineup.required.includes(p.id)).slice(0,4).map(p=>`<span title="${escape(p.player_name)}">${avatar({...p,player_id:p.id})}</span>`).join('')}</div><div class="lineup-budget">${money(50000 - lineup.salary)} remaining</div><div class="ui-recommendation-actions"><a href="/builder/?recommendation=${encodeURIComponent(lineup.id)}&slate=${encodeURIComponent(activeSlate)}">Export to Builder ↗</a>${lineup.required?.length ? `<a href="/builder/?recommendation=${encodeURIComponent(lineup.id)}&slate=${encodeURIComponent(activeSlate)}&stack=1">Build around stack ↗</a>` : ''}</div><p class="ui-lineup-core">${escape(lineup.players.filter(p=>lineup.required?.includes(p.id)).map(p=>p.player_name).join(" + ")||lineup.players.slice(0,3).map(p=>p.player_name).join(" · "))}</p></div><details class="ui-lineup-roster"><summary>View lineup & rationale</summary><table class="lineup-table" aria-label="${escape(lineup.title)} roster"><thead><tr><th scope="col">Slot</th><th scope="col">Player / matchup</th><th scope="col">Salary</th><th scope="col">Proj.</th></tr></thead><tbody>${lineup.players.map(p => `<tr><td>${escape(p.slot)}</td><td><span class="player-name">${escape(p.player_name)}${newsIcon(p.player_id,p.player_name)}${!['NONE', 'ACTIVE', 'HEALTHY'].includes(p.status.toUpperCase()) ? `<span class="injury-tag">${escape(p.status)}</span>` : ''}</span><span class="player-meta">${escape(p.team)} ${p.home_away === 'away' ? '@' : 'vs'} ${escape(p.opponent)} · ${escape(p.position)}</span></td><td>${money(p.salary)}</td><td>${p.points.toFixed(2)}</td></tr>`).join('')}</tbody></table><details class="lineup-detail"><summary>Why this build & data notes</summary>${explanation}<p>Totals are mean estimates, not floor or ceiling scores. ${identityCounts}. ${lineup.excluded.length ? 'Strategy exclusions apply, including roster changes when needed to keep builds distinct.' : 'Optimal for the included pool under the displayed roster constraints.'}</p><p>Scoring flags: ${escape([...new Set(lineup.players.flatMap(p => p.quality_flags))].join(', ').replaceAll('_', ' '))}.</p></details></details></article>`;
}
