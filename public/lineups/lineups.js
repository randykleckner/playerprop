...
function card(lineup) {
  ...
  return `<article class="lineup-card ${featured ? 'featured' : ''}" aria-labelledby="title-${escape(lineup.id)}"><div class="lineup-card-head"><h3 id="title-${escape(lineup.id)}">${escape(lineup.title)}</h3><div class="lineup-stats"><div><strong class="points">${lineup.points.toFixed(2)}</strong><span>PROJECTED PTS</span></div><div><strong>${money(lineup.salary)}</strong><span>TOTAL SALARY</span></div></div>${featured ? explanation : ''}<div class="lineup-budget">${money(50000 - lineup.salary)} remaining · ${provisional} provisional player joins</div></div><div><table class="lineup-table" aria-label="${escape(lineup.title)} roster"><thead><tr><th scope="col">Slot</th><th scope="col">Player / matchup</th><th scope="col">Salary</th><th scope="col">Proj.</th></tr></thead><tbody>${lineup.players.map(p => `<tr><td>${escape(p.slot)}</td><td><span class="player-name">${escape(p.player_name)}${!['NONE', 'ACTIVE', 'HEALTHY'].includes(p.status.toUpperCase()) ? `<span class="injury-tag">${escape(p.status)}</span>` : ''}</span><span class="player-meta">${escape(p.team)} ${p.home_away === 'away' ? '@' : 'vs'} ${escape(p.opponent)} · ${escape(p.position)}${p.provisional ? ' · ID pending' : ''}</span></td><td>${money(p.salary)}</td><td>${p.points.toFixed(2)}</td></tr>`).join('')}</tbody></table><details class="lineup-detail"><summary>Why this build & data notes</summary>${explanation}<p>Totals are mean estimates, not floor or ceiling scores. ${provisional} identities use unverified name, team and position matches. ${lineup.excluded.length ? 'Strategy exclusions apply, including roster changes when needed to keep builds distinct.' : 'Optimal for the included pool under the displayed roster constraints.'}</p><p>Scoring flags: ${escape([...new Set(lineup.players.flatMap(p => p.quality_flags))].join(', ').replaceAll('_', ' '))}.</p></details></div></article>`;
}
async function load() {
  $('lineup-content').hidden = true;
  $('lineup-empty').hidden = true;
  try {
    const response = await fetch('/lineups/latest.json', { cache: 'no-store' });
    ...
    $('slate-details').textContent = `${date(data.slate.start_time)} · ${data.slate.game_count} games · Draft group ${data.slate.draft_group_id}`;
    if (locked) {
