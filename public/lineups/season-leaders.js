...
try{const r=await fetch('/lineups/season-leaders.json',{cache:'no-store'});if(!r.ok)throw Error();const d=await r.json();
 ...
 document.getElementById('season-leader-rows').innerHTML=`<table class="lineup-table"><thead><tr><th>Rank</th><th>Player</th><th>Team</th><th>${esc(metrics[metric])}</th></tr></thead><tbody>${rows.map((p,i)=>`<tr><td>${i+1}</td><td><div class="ui-player-cell">${avatar(p,'xs')}<button data-research-player="${esc(p.player_id)}">${esc(p.player_name)}</button></div></td><td>${esc(p.team)}</td><td>${p[metric].toLocaleString()}</td></tr>`).join('')}</tbody></table>`;};select.onchange=render;render();
}catch{document.getElementById('season-leader-status').textContent='Actual season statistics unavailable. Other research sources remain available.';}
