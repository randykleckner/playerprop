...
try{
 ...
 const render=()=>{
  ...
  $('rows').innerHTML=rows.map(r=>`<tr><td>${esc(r.player_name)}</td><td>${esc(r.team)} ${esc(r.position)}</td><td>${esc(r.confidence)}</td><td>${esc(JSON.stringify(r.external_ids))}</td><td>${esc(r.canonical_player_id||r.candidates.join(', ')||'No candidate')}</td><td>${evidenceHtml(r.review_evidence)}</td></tr>`).join('');
 };
