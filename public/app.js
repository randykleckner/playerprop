import {avatar,teamMark,positionBadge,sportsbookBadge as bookLabel} from '/ui/components.js?v=ui-polish-5';
import {propTable} from '/ui/props-view.js?v=ui-polish-5';
import { propCard, flipCard } from "/prop-card.js";
import { normalizeGames, matchGame, showLock, weatherText, isUpcomingSignal } from "/game-context.js?v=upcoming-1";
const endpoint = "/api/signals/live?historySeason=2025&limit=100&view=doctor-chart-v1";
const state = { signals: [], capturedAt: null, media: {}, teams: {}, games: [], notebook: {} };
...
function qualifyingSignals() { return state.signals.filter((signal) => isUpcomingSignal(signal) && (signal.direction === "over" || signal.direction === "under")).sort((a, b) => b.confidence - a.confidence || Math.abs(b.score) - Math.abs(a.score)); }
...
function render() {
 ...
 elements.empty.textContent=`No usable upcoming player props. Last sportsbook import: ${state.capturedAt?kickoff(state.capturedAt):'not available'}. The odds feed needs a fresh import; started games and undated lines are excluded.`;
...
function showProp(id){const signal=state.signals.find(s=>storyId(s)===id);if(!signal||!isUpcomingSignal(signal))return;const context=matchGame(signal,state.games);document.getElementById('prop-detail-content').innerHTML=`<p class="eyebrow">${escapeHtml(signal.marketLabel)} · ${escapeHtml(signal.playerTeam)}</p><div class="ui-player-hero">${avatar({player_id:signal.playerId,player_name:signal.playerName,team:signal.playerTeam,position:signal.position},'lg')}<div><h2>${escapeHtml(signal.playerName)}</h2><p>${teamMark(signal.playerTeam,'sm')} ${escapeHtml(signal.playerTeam)} ${positionBadge(signal.position)} vs ${escapeHtml(signal.opponentTeam)}</p></div></div><div class="ui-metric-strip"><div class="ui-metric"><span>Sportsbook line</span><strong>${number(signal.line)}</strong></div><div class="ui-metric"><span>Recent average</span><strong>${number(signal.recentAverage)}</strong></div><div class="ui-metric"><span>Confidence score</span><strong>${number(signal.confidence)}</strong></div></div><p>${escapeHtml(signal.summary||fact(signal))}</p><p>${escapeHtml(weatherText(context))} · ${escapeHtml(kickoff(signal.commenceAt||context?.start))}</p><p>${bookLabel(signal.sportsbook)}</p><details><summary>Source & methodology</summary><p>Line capture: ${escapeHtml(signal.oddsCapturedAt||signal.capturedAt||signal.fetchedAt||'Not supplied by this feed')}. Confidence is not a probability of winning.</p></details><p><a href="/newsroom/?player=${encodeURIComponent(signal.playerId)}">Injuries & News ↗</a> · <a href="/simulation/?player=${encodeURIComponent(signal.playerId)}">Simulate player ↗</a></p><details><summary>Matchup chart & evidence</summary>${plaque(signal,0)}</details>`;document.getElementById('prop-detail').dataset.kickoff=signal.commenceAt;document.getElementById('prop-detail').showModal();}
...
async function load() { elements.error.hidden = true; const notesLoad = fetch("/doctor-chart-notes.json", {cache:"no-store"}).then(r => r.ok ? r.json() : {}).then(notes => { state.notebook = notes; }).catch(() => {}); const mediaLoad = Promise.all([fetch("/player-media.json", { cache:"force-cache" }), fetch("/team-media.json", { cache:"force-cache" })]).then(async ([players, teams]) => { if (players.ok) state.media = await players.json(); if (teams.ok) state.teams = await teams.json(); }).catch(() => {}); try { const response = await fetch(endpoint, { cache:"no-store" }); if (!response.ok) throw new Error(); const data = await response.json(); await Promise.all([mediaLoad, notesLoad]); state.signals = data.signals || []; state.capturedAt=data.capturedAt??null; render(); await loadGameContext(); render(); } catch (cause) { try { const fallback = await fetch("/fallback-signals.json", { cache:"no-store" }); if (!fallback.ok) throw cause; const data = await fallback.json(); await Promise.all([mediaLoad, notesLoad]); state.signals = data.signals || []; state.capturedAt=data.capturedAt??null; render(); await loadGameContext(); render(); elements.error.hidden = false; elements.error.textContent = "Live database refresh is temporarily unavailable. Showing verified upcoming games from the saved snapshot, if available."; } catch { elements.error.hidden = false; elements.error.textContent = "Unable to load the research desk."; } } }
...
// Expire quotes even when the board stays open across kickoff or resumes from sleep.
function expireStartedGames(){render();const dialog=document.getElementById('prop-detail');if(dialog?.open&&!isUpcomingSignal({commenceAt:dialog.dataset.kickoff}))dialog.close();}
setInterval(expireStartedGames, 30000);
document.addEventListener('visibilitychange',()=>{if(!document.hidden)expireStartedGames();});
window.addEventListener('focus',expireStartedGames);
