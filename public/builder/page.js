import {esc,money,decimal,option,avatar,positionBadge,icon} from '../ui/components.js?v=ui-polish-5';
...
function render(){
 ...
 $('roster').innerHTML=SLOTS.map((slot,i)=>{const p=picked[i];return `<div class="ui-slot ${replaceSlot===i?'ui-replacing':''}"><span class="ui-slot-label">${positionBadge(slot)}</span>${p?`<div class="ui-roster-player">${avatar(p)}<div><span class="ui-slot-name">${esc(p.player_name)}</span><span class="ui-slot-meta">${esc(p.team)} vs ${esc(p.opponent)} · ${money(p.salary)} · ${decimal(p.final_projection)} pts</span></div></div><div class="ui-slot-actions"><button class="ui-icon-button" data-lock="${esc(p.player_id)}" aria-pressed="${locks.has(p.player_id)}" aria-label="${locks.has(p.player_id)?'Unlock':'Lock'} ${esc(p.player_name)}">${icon(locks.has(p.player_id)?'lock':'unlock')}</button><button class="ui-icon-button" data-replace="${i}" aria-label="Replace ${esc(p.player_name)}">↔</button><button class="ui-icon-button" data-remove="${i}" aria-label="Remove ${esc(p.player_name)}">×</button></div>`:`<button class="ui-empty-slot" data-replace="${i}">+ Add ${({QB:'quarterback',RB:'running back',WR:'receiver',TE:'tight end',FLEX:'flex player',DST:'defense'})[slot]}</button>`}</div>`;}).join('');
