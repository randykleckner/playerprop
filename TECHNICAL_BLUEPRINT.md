...
## V2.0-C — Personnel-aware plays and fantasy outcomes

The experimental Drive Lab now supports V2 Base, V2 Personnel and matched comparison alongside foundation/legacy engines. Optional resolver and observer hooks reuse the same state/clock implementation. A separate seeded allocation stream assigns each rush/target to one canonical offensive player; TD ownership and shared DK scoring derive from those plays. Base at zero influence reproduces prior empirical game results. Personnel applies centrally configured, capped pressure/completion/histogram-shape changes using existing Madden units and an evidence-qualified receiving composite.
...
UI follow-up: shared `redesign.css` now defines the projection/leader overview grid and white Drive Lab matchup rows. Matchup portraits and logos use the existing canonical-ID media catalog and its image fallbacks; accessible names preserve player identity when visual labels are omitted. Cache versions were advanced through navigation, shell, stylesheet and changed page modules to prevent the old presentation surviving reloads. No model or data-ingestion changes.

### September 15 ingestion authentication recovery

The local saved token was present with 0600 permissions and no environment override. Secret updates alone still produced 401 responses; a full Worker deployment with preserved variables restored access using the same saved credential. This establishes the recovery sequence, not a proven Cloudflare internal root cause. Both admin routes now return safe failure codes for a missing header, missing binding, or credential mismatch without exposing token values. The CLI reports these distinctions instead of reflexively asking the user to reconfigure. Verified account usage and a guarded refresh succeeded, importing 1,558 quotes across 16 events and producing 60 upcoming modeled props at 2026-09-15T21:59:23.218Z. No paid upgrade or quota bypass.

September 15 follow-up: Drive Lab uses `current-games.js` and current research rather than `examples.json`. `refresh_season_leaders.py` runs independently during the twice-daily research refresh and preserves the last good actual-stat snapshot on failure. `newsroom/injury_wire.py` adds a structured ESPN injury adapter with exact external ID/team matching, source timestamps, return-estimate labels, and fail-closed schema/season handling. `refresh_newsroom.py` preserves full brief history before current-story deduplication. No additional paid-provider allowance, production Cron, or database migration is required.
