...
### Saved lineup review and live game context

DFS Daily keeps locked recommendations visible with a REVIEW ONLY banner. It preserves original projected points and salaries and hides builder-export actions for locked or explicitly archived builds. `/lineups/archive.json` indexes immutable research bundles; `?bundle=<id>` bookmarks a specific build. The scheduled public refresh rebuilds the archive index, including when the current slate is already locked. Older builds remain selectable after the latest slate changes.

The page reads ESPN's public NFL scoreboard once per minute while visible, scoped to the selected lineup teams and slate dates, and links to each box score. These are live **game** scores, not verified DraftKings player/lineup totals. Failure retains the prior display with an explicit stale notice. Player fantasy scoring and hindsight optimization are not implemented by this game-score panel. A proper hindsight result must use finalized, canonical player actuals (including DST scoring and stat corrections), the original complete salary/eligibility pool, and the original roster constraints. The best realized result among six recommendations is a different result from the optimal possible slate lineup.

### Slate leaders and matchup discovery — September 15, 2026

DFS Daily prioritizes the six recommended builds, then a top-ten projected-stat table with position filters and DK points, points per $1,000, targets, carries and pass attempts. Rankings come from the same immutable simulation bundle as the selected lineup build, including archived builds. They are projections, not season actuals. Inactive and unresolved players, duplicate canonical IDs, and missing/nonpositive metrics are excluded. Missing salaries cannot yield value rankings. Live game scores now sit at the bottom, after shared scenario results.

Drive Lab begins with Favorable matchups: saved sportsbook game totals, a close-game view (absolute spread at most four points), and team implied totals. Highlights show three projected player leaders and market source/capture time. Started games are excluded; expired snapshot status remains visible. These ranks do not assert defensive weakness, individual coverage, injury adjustment, or calibrated stack correlation. They use no additional external API quota. The diagnostic game selector remains independent.

Design refinement: the overview pairs Best Overall with a compact, scrollable stat-leader table, followed by two floor cards and a full-width ceiling grid. Narrow screens stack these sections. Drilldown views retain their existing layouts. Drive Lab uses the shared white workspace styling, compact team logos and player portraits with accessible names/tooltips. Market capture timestamps and provider names are removed from each matchup row; freshness failures remain visible and methodology remains in the disclosure. The archived matchup hero uses smaller logos.

### Current-week games, actual season leaders, and injury history

Drive Lab now resolves games directly from the current immutable research bundle, joins historical team profiles, and excludes kickoffs at or before the current time. It no longer loads the frozen example matchups. Personnel simulations fail closed when the availability snapshot belongs to another season/week; the empirical team engine remains available and clearly separate from current-week personnel evidence.

Recommended Lineups adds league-wide season actuals (passing yards/TDs, rushing yards/TDs, receiving yards, receptions), separate from salary-pool projections. NFLverse regular-season weekly records are aggregated by canonical ID with player/game deduplication, excluding the selected research week and later weeks. The through-week and season remain visible; old data is not relabeled as fresh after a refresh failure.

Newsroom now includes the public ESPN league injury wire, joined through ESPN IDs and canonical team agreement. Structured injury statuses and provider return dates become brief availability entries. Dates are estimates, not confirmed clearance. Previously captured briefs are preserved in a public history archive and can be searched with the History selector; historical entries are explicitly labeled. Missing/current official NFL injury reports are still flagged and do not automatically change projections.
