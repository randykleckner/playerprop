...
Pre-implementation audit: V2.0-B has seven unit composites (pass protection, run blocking, pass rush, run front, secondary man/zone/overall), detailed EA attributes, NFLverse canonical roster status and rank-one base depth roles. The latest personnel snapshot contains 2,364 EA players (1,900 strongly corroborated; 320 provisional; 144 unresolved). Unit evidence is complete for 179/224 outputs. Madden has no influence in the existing resolver.

`engine.ts` owns state transitions, possession, clock, decision rules and team totals. `resolvePlay` samples sacks, interceptions, completion and RUN/completed-PASS empirical histograms; rush/pass histograms are league distributions. State includes down, distance, field, quarter/clock, score and possession. No individual offensive players currently own plays. `browser-worker.ts` summarizes game scores and per-team rates and retains only the optional first-game trace. Drive Lab currently selects foundation A or current empirical B. Existing shared `dkScore`, seeded random generator, summarizer and personnel builder will be reused. V1 will remain untouched. The zero-influence empirical control will retain the existing play RNG sequence, with player allocation on its own stream.

V2.0-C is implemented through optional resolver/observer hooks rather than a duplicate clock engine. `player-engine.ts` adds canonical play recipients and box scores; `personnel-influence.ts` adds explicit bounded pressure, completion and histogram-shape interventions; `player-bulk.ts` produces player/team quantiles and matched comparisons. V2 Base forces zero influence and preserves the existing empirical outcomes. V2 Personnel is independently selectable at zero/low/standard/high influence; the original foundation and legacy empirical choices remain.

The player snapshot contains 447 canonical offensive players across 32 teams; 359 meet the detailed-output criteria and the remaining contributors retain actual identities with capped residual opportunities. Current depth identifies QBs, recency-weighted NFLverse PBP supplies usage, ESPN projections/depth supply flagged priors, and existing matched DK salary snapshots support value probabilities. Madden never determines carry/target volume. The pressure signal is explicitly a sack/hit/scramble proxy, and no shadow coverage is inferred.

Drive Lab now shows Fantasy Player Outcomes, expandable stat/DK quantiles, first-game player box scores with named trace recipients, and same-seed Base/Personnel deltas. Only the Personnel selection receives Madden adjustments. V1 remains unchanged. The benchmark tracks 27 players in BUF/HOU, runs 10,000 games in about four seconds per engine, and stores no bulk play traces.

**183 tests pass (100 Node, 83 Python; 21 new)**, including exact zero-control equivalence, seeded determinism, opportunity/yard/TD conservation, scoring bonuses, probability bounds and UI output. See [player-simulation-v2.md](player-simulation-v2.md) for formulas, configuration, limitations, the first-game box score, 10,000-game player distributions, performance and Base/Personnel deltas. Machine-readable receipt: [simulation-v2-c-benchmark.json](simulation-v2-c-benchmark.json).
