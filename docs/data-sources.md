...
### Props outage audit — September 13, 2026

The live D1 odds pool contained 2,731 quotes for 32 events, all captured September 3 at 17:59:48 UTC. Every joined event kickoff was null. The upcoming-only board therefore had zero eligible quotes. The public refresh job reads the existing API; it does **not** acquire fresh SportsGameOdds data. A successful HTTP response must not be interpreted as successful sportsbook ingestion.

The provider importer now reads `status.startsAt` before the older top-level timestamp alternatives and excludes started/cancelled/undated events. The API rechecks kickoff even for cached responses. `feedHealth` and `/props-status.json` distinguish current, stale, empty, and failed reads. A snapshot refresh returns nonzero for an empty or >24-hour-old quote feed. Original capture times remain unchanged; old lines are never relabeled as current.

An explicitly authorized provider refresh can run with `python3 scripts/refresh_sportsbook_props.py`. It calls the existing protected Worker endpoint using `PLAYERPROP_INGEST_TOKEN`, or prompts for the existing token without echo in an interactive terminal. The sportsbook API key stays in the Worker. This command consumes the existing provider allowance and is intentionally excluded from the public-only scheduled task. No purchases, secret rotation, migration or Cron changes are made. Unavailable provider access remains a real coverage limitation: the system cannot guarantee current sportsbook lines when its source is unavailable.

User authorization on September 13: existing SportsGameOdds free allowance may be used for current and twice-daily odds refreshes, with no purchases or upgrades. Public documentation lists 2,500 event objects/month (not individual player lines); account-specific limits must be checked before enabling recurring pulls. At a maximum of 32 events per run, two runs/day for 31 days would consume up to 1,984 event objects, excluding usage checks, retries and other clients. Local credential setup: `python3 scripts/refresh_sportsbook_props.py --configure-token`; owner-only `.props-ingest-token` is ignored by Git. Configuration alone makes no provider call. Recurring odds pulls remain gated on credential setup and quota verification.

Recovery verified September 13 at 18:17 UTC: the local ingest token authenticated, the provider reported an active free account with 32/2,500 monthly objects consumed before the request, and one bounded request imported 3,804 quotes across 32 returned events. The live board produced 57 eligible upcoming signals. The request required an identifiable User-Agent for the local authenticated client.

The protected refresh now checks the actual free-tier quota before requesting odds, supports the observed hyphenated quota fields and documented camel-case fields, refuses unknown/inactive/non-free limits, and retains a 100-object monthly reserve (including the next maximum 32-event response). A six-hour successful-import cooldown prevents repeated clicks from consuming allowance. Requests cover the next seven days, with at most one 32-event page and no automatic pagination. Usage checks also consume provider requests; other API clients can consume the same quota. The guard is conservative rather than an atomic reservation shared with external clients. The approved Mac task may now invoke `scripts/refresh_sportsbook_props.py` before the public refresh. No new production Cron or purchases are enabled.
